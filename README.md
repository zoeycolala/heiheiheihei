# heiheiheiheiimport { useState } from "react";
...
const LUCKY_COLORS = ["深紫","星蓝","玫瑰金","翡翠绿","珍珠白","烟熏橙","霞光红","星空灰","薄荷绿","琥珀黄","丁香紫","冰蓝"];
...
请严格只返回如下JSON格式，不要有任何多余文字或markdown符号：
{
  ...
  return (
    <div style={{ padding: "20px 16px", maxWidth: 480, margin: "0 auto", fontFamily: "sans-serif" }}>
      <h2 style={{ fontSize: 16, fontWeight: 500, marginBottom: 4, color: "var(--color-text-primary)" }}>每日星座运势</h2>
      <p style={{ fontSize: 13, color: "var(--color-text-secondary)", marginBottom: 20 }}>选择星座，生成卡片，截图发群</p>

      <div style={{ marginBottom: 14 }}>
        <label style={{ fontSize: 12, color: "var(--color-text-secondary)", display: "block", marginBottom: 6 }}>日期</label>
        <input
          type="date"
          value={date}
      ...
      <div style={{ marginBottom: 20 }}>
        <label style={{ fontSize: 12, color: "var(--color-text-secondary)", display: "block", marginBottom: 8 }}>星座</label>
          ...
          {SIGNS.map((s, i) => (
            <div
              ...
              style={{
                ...
                padding: "8px 4px",
                textAlign: "center",
                border: sign?.name === s.name ? `2px solid ${COLORS[i]}` : "1.5px solid var(--color-border-tertiary)",
                background: sign?.name === s.name ? `${COLORS[i]}18` : "var(--color-background-secondary)",
                transition: "all 0.15s",
              }}
            >
              <div style={{ fontSize: 18 }}>{s.emoji}</div>
              <div style={{ fontSize: 11, color: "var(--color-text-primary)", marginTop: 2 }}>{s.name}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <button
        ...
        style={{
          ...
          borderRadius: 10,
          border: "none",
          background: sign ? "#5c6bc0" : "var(--color-background-tertiary)",
          color: sign ? "#fff" : "var(--color-text-tertiary)",
          fontSize: 14,
          fontWeight: 500,
      ...
      {result && sign && (
        <CardView result={result} sign={sign} date={date} />
      )}
    </div>
  );
}
